...
const Time = () => {
  ...
  const [time,setTime]=useState({work:1500,break:300})
  ...
  return (!state.pomodoroStatus?
    <div className='flex flex-col py-4 justify-center items-center'>
      <div onClick={()=>toggleActive("work")} className={'flex justify-center gap-2  border-white/50 w-fit mb-[2px] '+(clicked==="work"&&"mb-0 border-b-[2px]")}>
        <p className='text-white text-8xl'>{active==='work'?formatter(timeLeft).minutes:formatter(time.work).minutes}</p>
        <p className='text-white text-8xl'>:</p>
        <p className='text-white text-8xl'>{active==='work'?formatter(timeLeft).seconds:formatter(time.work).seconds}</p>
      </div>
      <div onClick={()=>toggleActive("break")} className={'flex justify-center gap-2  border-white/50 w-fit mt-2 mb-[32px]  '+(clicked==="break"&&"border-b-[2px] mb-[30px]")}>
        <p className='text-white text-4xl'>{active==='break'?formatter(timeLeft).minutes:formatter(time.break).minutes}</p>
        <p className='text-white text-4xl'>:</p>
        <p className='text-white text-4xl'>{active==='break'?formatter(timeLeft).seconds:formatter(time.break).seconds}</p>
      </div>
      <div className='flex items-center justify-between gap-4'>
        <button onClick={()=>toggleTime("minus")}><img src='/svg/minus.svg' className='w-16' /></button>
        <button onClick={toggleTimer} className=' border-[4px] border-white rounded-lg w-[120px] h-16 text-white text-2xl text-center align-middle'>
          {ticking?'STOP':'START'}  
        </button>
        <button onClick={()=>toggleTime("plus")}><img src='/svg/plus.svg' className='w-16' /></button>
    </div>
    </div>:
