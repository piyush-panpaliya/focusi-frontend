...
const index = () => {
  const {setUI,state}=useContext(AppContext)
  const handler=(a)=>{
    setUI(a,!(state.UI[a]))
  }
  return (
    ...
    <button onClick={()=>handler("lb")} className='w-16 h-16  rounded-full flex items-center justify-center'><img src='/svg/clock.svg' className=' w-16'/></button>
